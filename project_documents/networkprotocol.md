...
| `<tile>`                                                       | Repräsentation eines Spielsteins;<br/>`<zahl>:<farbe>`, wobei `<zahl>` eine Zahl zwischen `0` und `13` und `<farbe>` entweder `RED`, `BLUE`, `YELLOW` oder `BLACK` ist. Speziell: bei der Zahl 0 handelt es sich um den Joker, egal in welcher Farbkombination. |
